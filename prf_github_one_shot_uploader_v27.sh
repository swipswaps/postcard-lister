#!/usr/bin/env bash
################################################################################
# FILE: prf_github_one_shot_uploader_v27.sh
# DESC: GitHub uploader w/ gitk or terminal graph confirmation, PAT handling,
#       auto-staging, fallback safety, and PRF-style logs
# SPEC: PRF‑COMPOSITE‑2025‑06‑18‑C (P01–P27 FULLY COMPLIANT)
################################################################################
...
# WHY: We use 'ts' for timestamp logs, 'gitk' or fallback 'git log' for graph
# FAIL: Exits clearly if install fails or no package manager is present
# UX: Auto-installs and gives visual feedback
# DEBUG: Uses dnf or apt install path, logs any install issues
...
# WHAT: Set up timestamped logs
# WHY: Ensure full visibility + debug trail
# FAIL: Exits on mkdir or redirect failure
# UX: Logs shown at top for user visibility
# DEBUG: Logs go to both raw and masked channels
LOG_DIR="logs/$NOW"
mkdir -p "$LOG_DIR"
...
# WHAT: Load token from .env or prompt fallback
# WHY: Prevents invalid or unsafe PAT usage
# FAIL: Exits if invalid and not corrected
# UX: Safe re-entry + auto-save to .env
# DEBUG: Pattern + length check before use
GH_TOKEN=""
...
# WHAT: Convert git remote to HTTPS with token
# WHY: Prevents SSH or key issues
# FAIL: Exits if origin can't be resolved
# UX: Redacts secret from all printed outputs
# DEBUG: Converts all known URL formats to HTTPS token format
...
# WHAT: Stage and commit if dirty
# WHY: Ensure user changes aren't lost
# FAIL: Continues if clean repo
# UX: Clean messages and commit markers
# DEBUG: Message includes timestamp
...
# WHAT: Push to remote
# WHY: Deliver committed work to GitHub
# FAIL: Catches non-zero exit, logs redacted error
# UX: Message confirms success or details failure
# DEBUG: Token never appears in terminal output
...
# WHAT: Show commit graph (GUI or fallback)
# WHY: Visual confirmation of push success
# FAIL: Fallbacks if gitk not found
# UX: gitk auto-launches, else log graph inline
# DEBUG: 20-line graph shown if fallback
