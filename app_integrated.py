...
class GitHubUploadWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Create enhanced script with granular GitHub progress monitoring
            enhanced_script = f"""#!/bin/bash
# Enhanced GitHub upload with granular progress and file transfer monitoring
...
echo "📤 Starting enhanced upload with granular progress..."

# Check what files will be uploaded
echo "📋 Files to be committed:"
git diff --cached --name-status 2>/dev/null || git diff --name-status || echo "No changes to commit"

echo "📊 Repository size before upload:"
du -sh .git 2>/dev/null || echo "Cannot determine repo size"

# Add files with verbose output
echo "📁 Adding files to staging area..."
git add . --verbose 2>&1 || echo "Git add failed"

# Commit with detailed output
echo "💾 Creating commit..."
git commit -m "{self.commit_message}" --verbose 2>&1 || echo "Nothing to commit"

# Show commit details
echo "📋 Commit details:"
git show --stat HEAD 2>/dev/null || echo "Cannot show commit details"

# Push with GitHub CLI (which has working authentication)
echo "🚀 Pushing to GitHub with GitHub CLI..."
echo "📡 Using GitHub CLI for authenticated push..."

# First try GitHub CLI push (which works)
if command -v gh >/dev/null 2>&1; then
    echo "✅ GitHub CLI available, using authenticated push..."
    gh repo sync --source . --verbose 2>&1 || echo "GitHub CLI sync failed, trying git push..."

    # Also try direct git push for progress monitoring
    echo "📊 Attempting git push for progress details..."
    GIT_CURL_VERBOSE=1 git push --progress --verbose origin main 2>&1 || echo "Git push failed (expected if using token auth)"
else
    echo "⚠️ GitHub CLI not available, using git push..."
    GIT_CURL_VERBOSE=1 git push --progress --verbose origin main 2>&1 || echo "Push failed"
fi

echo "✅ Upload operations completed"
echo "📊 Final repository status:"
git status --porcelain || echo "Git status failed"
echo "🔗 Remote repository info:"
git remote -v 2>/dev/null || echo "Cannot show remote info"
echo "📈 Recent commits:"
...
"""
            ...
            try:
                ...
                # Monitor process with extended timeout for git operations
                timeout_seconds = 180  # Extended to 3 minutes for git log operations
                poll_interval = 2
                elapsed = 0
                ...
                while process.poll() is None and elapsed < timeout_seconds:
                    ...
                    if elapsed % 10 == 0:  # Every 10 seconds
                        ...
                        # Enhanced network and git operation monitoring
                        try:
                            ...
                            # Monitor git operations and file transfers
                            if elapsed % 15 == 0:  # Every 15 seconds
                                # Check if git is actively transferring data
                                git_processes = sp.run(['pgrep', '-f', 'git'], capture_output=True, text=True, timeout=1)
                                if git_processes.returncode == 0 and git_processes.stdout.strip():
                                    self.network_status.emit("📤 Git operations active")

                                    # Check network traffic on git connections
                                    ss_result = sp.run(['ss', '-i', 'dst', ':443'], capture_output=True, text=True, timeout=2)
                                    if ss_result.returncode == 0:
                                        for line in ss_result.stdout.split('\n'):
                                            if 'bytes_sent' in line or 'bytes_received' in line:
                                                self.verbatim_output.emit(f"📊 NETWORK: {line.strip()}")
